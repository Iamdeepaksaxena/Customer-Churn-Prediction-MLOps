...
# setting up here params for yaml
def load_params(params_path: str) -> dict:
    ...
    try:
        ...
        return params
    # here that parmsfile is return all parameters that we give in that file
...
def save_data(train_data:pd.DataFrame, test_data:pd.DataFrame, data_path:str) -> None:
    ...
    try:
        ...
        logger.error('Unexpected errors occurs while saving the data: %s',e)
        raise
...
def main():
    try:
        ...
        test_size = 0.20
        data_path ='Telcom Data.csv'
        df = load_data(data_url= data_path)
        final_df = preprocess_data(df)
        train_data, test_data = train_test_split(final_df, test_size = test_size, random_state = 42)
        save_data(train_data, test_data, data_path='./data')
    except Exception as e:
        logger.error("Data ingestion process is failed: %s",e)
        print(f'Error{e}')
